...
def db_get_member_channel(guild_id: int, user_id: int) -> Optional[int]:
    with sqlite3.connect(DB_PATH) as con:
        cur = con.cursor()
        cur.execute(
            "SELECT channel_id FROM member_channels WHERE guild_id=? AND user_id=?",
            (guild_id, user_id),
        )
        row = cur.fetchone()
...
def db_set_member_channel(guild_id: int, user_id: int, channel_id: int):
    with sqlite3.connect(DB_PATH) as con:
        cur = con.cursor()
        cur.execute(
            "INSERT OR REPLACE INTO member_channels (guild_id, user_id, channel_id) VALUES (?, ?, ?)",
...
def db_get_submission(submission_id: int):
    with sqlite3.connect(DB_PATH) as con:
        cur = con.cursor()
        cur.execute("""
            SELECT id, guild_id, user_id, farm_channel_id, item, quantity, image_url, status
            FROM submissions
            WHERE id=?
        """, (submission_id,))
        return cur.fetchone()
...
class FarmModal(discord.ui.Modal, title="📤 Enviar Farm"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async def expire():
            ...
            if cur and cur[2] == self.channel_id:
...
class ApprovalView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        # RESPONDE a interação imediatamente (evita “interaction failed”)
        ...
        if status != "PENDING":
            await interaction.followup.send(f"Essa solicitação já está **{status}**.", ephemeral=True)
            return
        ...
        # apaga o “registro” (mensagem de aprovação)
        try:
            await interaction.message.delete()
        except discord.Forbidden:
            # se não conseguir apagar, remove botões (pra não clicar de novo)
    ...
    async def reject(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        user = await self.bot.fetch_user(user_id)
        dm = discord.Embed(title="❌ Farm Rejeitado", description="Seu envio foi rejeitado. Confira e envie novamente.")
        dm.add_field(name="Item", value=item, inline=True)
        dm.add_field(name="Quantidade", value=str(qty), inline=True)
        try:
            await user.send(embed=dm)
...
class FarmBot(commands.Bot):
    def __init__(self):
        ...
        intents.message_content = False  # não precisamos ler texto, só anexos
